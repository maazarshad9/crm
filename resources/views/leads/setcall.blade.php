@extends('layouts.app', ['title' => __('Calling Management')])

@section('content')
@include('leads.partials.header', ['title' => __('Add Call')])   
            ...
            <div class="card bg-secondary shadow">
                <div class="card-header bg-white border-0">
                    <div class="row align-items-center">
                        <div class="col-8">
                            <h3 class="mb-0">{{ __('Calling Management') }}</h3>
                        </div>
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('lead.storecalling', $lead) }}" autocomplete="off">
                        @csrf
                        @method('put')

                        <h6 class="heading-small text-muted mb-4">{{ __('Calling information') }}</h6>
                        <div class="pl-lg-4">
						    <div class="form-group{{ $errors->has('calling_date') ? ' has-danger' : '' }}">
							<label class="form-control-label" for="input-calling_date">{{ __('Calling Date') }}</label>
                                <div class="input-group input-group-alternative">
                                    <div class="input-group-prepend">
                                        <span class="input-group-text"><i class="ni ni-calendar-grid-58"></i></span>
                                    </div>
                                    <input name="calling_date" class="form-control datepicker {{ $errors->has('calling_date') ? ' is-invalid' : '' }}" placeholder="{{ __('Calling Date') }}" placeholder="Select date" type="text" required>
							...
							<div class="form-group{{ $errors->has('calling_time') ? ' has-danger' : '' }}">
                                <div style="position: relative">
								 <label class="form-control-label" for="input-calling_time">{{ __('Calling Time') }}</label>
								  <input name="calling_time" id="input-calling_time" class="timepicker form-control" type="text" placeholder="{{ __('Calling Time') }}" required autofocus>
								</div>
                            ...
                            <div class="form-group">
								...
								<input type="hidden" name="status" class="form-control" placeholder="{{ __('Status') }}" value="calling" >
                            </div>
